(function (window, angular, undefined) {
      ...
      var linker = function postLink(scope, lElement, attrs) {
        ...
        scope.$watch('isotopeItemFilter', function (newval, oldval) {
          ...
          if (newval != null || newval !== '') {
            ...
            if (val) {
              lElement.children('li').filter(function () {
                return $(this).find('a').attr('data-title').toLowerCase().indexOf(val) === -1;
              }).addClass('filter-out');
            }
          }
        });
  ...
  angular.module('playerElement', []).directive('playerElement', [
    '$parse',
    '$timeout',
    function ($parse, $timeout) {
      return {
        ...
        scope: true,
        replace: false,
        template: '<audio preload="none" controls loop>' + '<source src = "assets/media/Barthezz-Infected.mp3"/>' + '<source src = "assets/media/Barthezz-Infected.ogg"/>' + '</audio>',
  ...
  ;
  angular.module('slider', []).directive('slider', [
    '$parse',
    function ($parse) {
      return {
        restrict: 'A',
  ...
  angular.module('subnav', []).directive('subnav', [
    '$parse',
    function ($parse) {
      return {
        restrict: 'A',
  ...
  function gridbuilder() {
    var $grid = $('#og-grid'), $items = $grid.children('.grid-item'), current = -1, previewPos = -1, scrollExtra = 0, marginExpanded = 10, $window = $(window), winsize, $body = $('html, body'), transEndEventNames = {
    ...
    c.fn.imagesLoaded = function (f) {
      ...
      function k(b, a) {
        b.src === m || -1 !== c.inArray(b, l) || (l.push(b), a ? h.push(b) : j.push(b), c.data(b, 'imagesLoaded', {
          ...
          c(j),
          c(h)
        ]), e.length === l.length && (setTimeout(n), e.unbind('.imagesLoaded', p)));
      }
